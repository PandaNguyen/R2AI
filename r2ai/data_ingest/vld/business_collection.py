...
def classify_metadata(
    row: dict[str, Any],
    min_year: int,
    extra_support_keywords: Iterable[str] | None = None,
) -> tuple[bool, str]:
    ...
    support_hit = contains_any(search_text, SUPPORT_KEYWORDS) or contains_any(search_text, extra_support_keywords or ())
...
def load_effect_status_by_id(effect_metadata_path: Path | None) -> dict[int, str]:
    ...
    pf = _require_pyarrow().ParquetFile(effect_metadata_path)
