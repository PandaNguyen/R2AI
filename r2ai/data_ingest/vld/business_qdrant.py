...
except ImportError:  # pragma: no cover - optional data dependency.
    pq = None


def _require_pyarrow() -> Any:
    if pq is None:
        raise RuntimeError("pyarrow is required. Run: uv sync --extra data")
    return pq

from r2ai.data_ingest.vld.business_collection import (
    classify_metadata,
    effect_status,
...
from r2ai.data_ingest.vld.chunking import build_chunks_for_tree, make_qdrant_preview
from r2ai.data_ingest.vld.tree import build_tree


def iter_parquet_rows(path: Path, columns: list[str] | None = None) -> Iterator[dict[str, Any]]:
    pf = _require_pyarrow().ParquetFile(path)
...
def load_business_metadata(
    ...
    *,
    ...
    extra_support_keywords: list[str] | None = None,
    ...
    for row in iter_parquet_rows(metadata_path):
        ...
        if allowed_ids is not None and doc_id not in allowed_ids:
            continue
        keep, tier = classify_metadata(row, min_year=min_year, extra_support_keywords=extra_support_keywords)
        if not keep:
...
def build_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    ...
    metadata_by_id, metadata_report = load_business_metadata(
        ...
        extra_support_keywords=args.extra_support_keyword,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--extra-support-keyword",
        action="append",
        default=[],
        help="Additional metadata keywords that classify matching documents as support-tier candidates.",
    )
