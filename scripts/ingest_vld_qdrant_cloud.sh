...
EFFECT_METADATA_PATH="${R2AI_VLD_EFFECT_METADATA_PATH:-data/vietnam-legal-documentv2/legacy/metadata.parquet}"
MODEL_CACHE_DIR="${R2AI_MODEL_CACHE_DIR:-.cache/models}"
COLLECTION="${QDRANT_COLLECTION:-vld_business_law_v2}"
...
DENSE_MODEL="${R2AI_DENSE_MODEL:-AITeamVN/Vietnamese_Embedding_v2}"
SPARSE_MODEL="${R2AI_SPARSE_MODEL:-Qdrant/bm25}"
BATCH_SIZE="${R2AI_BATCH_SIZE:-64}"
UPSERT_BATCH_SIZE="${R2AI_UPSERT_BATCH_SIZE:-32}"
MAX_TEXT_TOKENS="${R2AI_MAX_TEXT_TOKENS:-2048}"
...
HNSW_M="${R2AI_HNSW_M-32}"
HNSW_EF="${R2AI_HNSW_EF-200}"
...
echo "  EFFECT_METADATA_PATH=$EFFECT_METADATA_PATH"
...
BUILD_ARGS=(
  ...
  --min-year "$MIN_YEAR"
  --effect-metadata-path "$EFFECT_METADATA_PATH"
